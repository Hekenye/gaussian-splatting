...
from utils.tracing_utils import trace_gaussians
...
def create_labels_for_OTF(gaussians: GaussianModel, camera, segmap):
    # for ray tracing
    ...
    focal = (fx, fy)
    principal = (W / 2, H / 2)
    C2W = camera.world_view_transform.transpose(0, 1).inverse()

    # get head and torso points mask, head include face and hair, torso include body-skin, clothes
    gaussians_pts = gaussians.get_xyz.detach()
    head_mask = trace_gaussians(gaussians_pts, segmap, H, W, focal, principal, C2W, batch_size=64, device='cuda')
    return head_mask
